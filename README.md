...
| 需求 | 说明 |
|---|---|
| 📁 三文件存档 | 保存裁判场景时自动生成 `Referee.json`（明文）+ `Blue.SpScn` + `Red.SpScn`（混淆），可读取任意视角，与桌面版 `Scenarios/` 目录互通 |
| 🕵️ 感知迷雾 | 可设置单位对蓝/红方可见性；保存红蓝存档时按 `PerceptionArray` 过滤，真实体现玩家视角（FogOfWar 引擎） |
| ⏱ 回合时间 | 回合时长自由填写 **XX分XX秒**，默认 3 分钟（`TurnInterval {Minutes, Seconds}`） |
| 👆 触摸交互 | 双指捏合缩放 / 单指拖拽平移 / 轻点选择 / 长按编辑（底部弹层） |
| ⚓ 规则引擎 | 鱼叉同步回合移动：转向前冲/分段/渐进式、加速两档、每 45° 转向损失、急舵、潜艇静航 |
| 🗺 地图 | 无图网格 / 位图地图（PNG + txt 比例尺） |
...
├── render/        # Camera 视口 / MapRenderer / UnitRenderer / TrackRenderer
...
- 保存：选择场景目录，自动生成三文件（`File` 字段分别为 Referee/Blue/Red）。
...
- [x] P0 数据地基：模型 / 编解码 / 三文件读写 / 回合时长分秒
- [x] P1 渲染浏览：手势画布 / 网格 / 军标 / 轨迹
- [x] P2 功能复刻：单位编辑 / 回合机制 / 感知迷雾 / 回合时间
- [ ] P3 打磨发布：横竖屏适配 / 场景库管理 / 回放
